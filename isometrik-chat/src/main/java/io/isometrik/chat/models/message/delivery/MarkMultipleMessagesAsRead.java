...
/**
 * The helper class to validate request params and make the mark multiple messages as read request and expose the parsed successful or error response.
 */
public class MarkMultipleMessagesAsRead {
  /**
   * Validate params.
   *
  ...
  public void validateParams(
      ...
      @NotNull final CompletionHandler<MarkMultipleMessagesAsReadResult> completionHandler,
      @NotNull RetrofitManager retrofitManager, @NotNull IMConfiguration imConfiguration,
      @NotNull final BaseResponse baseResponse, final @NotNull Gson gson) {
    String conversationId = markMultipleMessagesAsReadQuery.getConversationId();
    String userToken = markMultipleMessagesAsReadQuery.getUserToken();
    ...
    if (userToken == null || userToken.isEmpty()) {
      ...
      if(markMultipleMessagesAsReadQuery.getTimestamp()!=null) {
        bodyParams.put("timestamp", markMultipleMessagesAsReadQuery.getTimestamp());
      }
      Call<MarkMultipleMessagesAsReadResult> call =
          retrofitManager.getMessageService().markMultipleMessagesAsRead(headers, bodyParams);

      call.enqueue(new Callback<MarkMultipleMessagesAsReadResult>() {
        @Override
        public void onResponse(@NotNull Call<MarkMultipleMessagesAsReadResult> call,
            @NotNull Response<MarkMultipleMessagesAsReadResult> response) {
        ...
        @Override
        public void onFailure(@NotNull Call<MarkMultipleMessagesAsReadResult> call,
            @NotNull Throwable t) {
