...
/**
 * The helper class to validate request params and make the fetch message read status request and expose the parsed successful or error response.
 */
public class FetchMessageReadStatus {
  /**
   * Validate params.
   *
   ...
   * @param gson the gson
   */
  public void validateParams(@NotNull FetchMessageReadStatusQuery fetchMessageReadStatusQuery,
      @NotNull final CompletionHandler<FetchMessageReadStatusResult> completionHandler,
      @NotNull RetrofitManager retrofitManager, @NotNull IMConfiguration imConfiguration,
      @NotNull final BaseResponse baseResponse, final @NotNull Gson gson) {

    String conversationId = fetchMessageReadStatusQuery.getConversationId();
    String messageId = fetchMessageReadStatusQuery.getMessageId();
    Integer limit = fetchMessageReadStatusQuery.getLimit();
    Integer skip = fetchMessageReadStatusQuery.getSkip();
    String userToken = fetchMessageReadStatusQuery.getUserToken();
    ...
    if (userToken == null || userToken.isEmpty()) {
      ...
      Call<FetchMessageReadStatusResult> call =
          retrofitManager.getDeliveryStatusService().fetchMessageReadStatus(headers, queryParams);

      call.enqueue(new Callback<FetchMessageReadStatusResult>() {
        @Override
        public void onResponse(@NotNull Call<FetchMessageReadStatusResult> call,
            @NotNull Response<FetchMessageReadStatusResult> response) {
        ...
        @Override
        public void onFailure(@NotNull Call<FetchMessageReadStatusResult> call,
            @NotNull Throwable t) {
