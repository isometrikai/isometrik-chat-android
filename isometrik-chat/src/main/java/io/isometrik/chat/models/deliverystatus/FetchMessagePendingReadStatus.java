...
/**
 * The helper class to validate request params and make the fetch message pending read status request and expose the parsed successful or error response.
 */
public class FetchMessagePendingReadStatus {
  /**
   * Validate params.
   *
  ...
  public void validateParams(
      ...
      @NotNull final CompletionHandler<FetchMessagePendingReadStatusResult> completionHandler,
      @NotNull RetrofitManager retrofitManager, @NotNull IMConfiguration imConfiguration,
      @NotNull final BaseResponse baseResponse, final @NotNull Gson gson) {

    String conversationId = fetchMessagePendingReadStatusQuery.getConversationId();
    String messageId = fetchMessagePendingReadStatusQuery.getMessageId();
    Integer limit = fetchMessagePendingReadStatusQuery.getLimit();
    Integer skip = fetchMessagePendingReadStatusQuery.getSkip();
    String userToken = fetchMessagePendingReadStatusQuery.getUserToken();
    ...
    if (userToken == null || userToken.isEmpty()) {
      ...
      Call<FetchMessagePendingReadStatusResult> call = retrofitManager.getDeliveryStatusService()
          .fetchMessagePendingReadStatus(headers, queryParams);

      call.enqueue(new Callback<FetchMessagePendingReadStatusResult>() {
        @Override
        public void onResponse(@NotNull Call<FetchMessagePendingReadStatusResult> call,
            @NotNull Response<FetchMessagePendingReadStatusResult> response) {
        ...
        @Override
        public void onFailure(@NotNull Call<FetchMessagePendingReadStatusResult> call,
            @NotNull Throwable t) {
