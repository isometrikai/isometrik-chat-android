...
/**
 * The helper class to validate request params and make the fetch users request and expose the parsed successful or error response.
 */
public class FetchUsers {
  ...
  public void validateParams(@NotNull FetchUsersQuery fetchUsersQuery,
    ...
    Integer count = fetchUsersQuery.getCount();
    String searchTag = fetchUsersQuery.getSearchTag();
    ...
    if (pageToken != null) queryParams.put("pageToken", pageToken);
    if (count != null) queryParams.put("count", count);
    if (searchTag != null) queryParams.put("searchTag", searchTag);

    Call<FetchUsersResult> call = retrofitManager.getUserService().fetchUsers(headers, queryParams);
