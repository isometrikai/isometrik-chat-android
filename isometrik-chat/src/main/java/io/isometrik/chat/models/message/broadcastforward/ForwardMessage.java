...
/**
 * The helper class to validate request params and make the forward message request and expose the parsed successful or error response.
 */
public class ForwardMessage {
   ...
   * @param gson the gson
   */
  public void validateParams(@NotNull ForwardMessageQuery forwardMessageQuery,
      @NotNull final CompletionHandler<ForwardMessageResult> completionHandler,
      @NotNull RetrofitManager retrofitManager, @NotNull IMConfiguration imConfiguration,
      @NotNull final BaseResponse baseResponse, final @NotNull Gson gson) {
    Integer messageType = forwardMessageQuery.getMessageType();
    Boolean showInConversation = forwardMessageQuery.getShowInConversation();
    String body = forwardMessageQuery.getBody();
    Boolean encrypted = forwardMessageQuery.getEncrypted();
    List<String> userIds = forwardMessageQuery.getUserIds();
    List<String> conversationIds = forwardMessageQuery.getConversationIds();
    String deviceId = forwardMessageQuery.getDeviceId();

    EventForMessage eventForMessage = forwardMessageQuery.getEventForMessage();
    String customType = forwardMessageQuery.getCustomType();

    JSONObject metaData = forwardMessageQuery.getMetaData();
    List<Attachment> attachments = forwardMessageQuery.getAttachments();
    String userToken = forwardMessageQuery.getUserToken();
    List<String> searchableTags = forwardMessageQuery.getSearchableTags();
    ...
    if (userToken == null || userToken.isEmpty()) {
    ...
    } else if ((userIds == null || userIds.isEmpty()) && (conversationIds == null || conversationIds
        .isEmpty())) {
      completionHandler.onComplete(null, IsometrikErrorBuilder.IMERROBJ_USER_IDS_CONVERSATION_IDS_MISSING);
      ...
      if (userIds != null) bodyParams.put("userIds", userIds);
      if (conversationIds != null) bodyParams.put("conversationIds", conversationIds);
      ...
      Call<ForwardMessageResult> call =
          retrofitManager.getMessageService().forwardMessage(headers, bodyParams);

      call.enqueue(new Callback<ForwardMessageResult>() {
        @Override
        public void onResponse(@NotNull Call<ForwardMessageResult> call, @NotNull Response<ForwardMessageResult> response) {
        ...
        @Override
        public void onFailure(@NotNull Call<ForwardMessageResult> call, @NotNull Throwable t) {
