...
/**
 * The fragment to fetch list of members in a conversation with paging, search and pull to refresh
 * option and kickout member, grant/revoke admin privileges.
 */
public class MembersFragment extends Fragment implements MembersContract.View {
  ...
  private final ArrayList<MembersWatchersModel> members = new ArrayList<>();
  ...
  public View onCreateView(@NotNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    ...
    membersPresenter.initialize(conversationId, extras.getBoolean("isUserAnAdmin"), extras.getString("userId"));

    membersLayoutManager = new LinearLayoutManager(getActivity());

    ismFragmentMembersWatchersBinding.rvMembersWatchers.setLayoutManager(membersLayoutManager);

    membersAdapter = new MembersWatchersAdapter(getActivity(), members, this, null);

    ismFragmentMembersWatchersBinding.rvMembersWatchers.addOnScrollListener(membersOnScrollListener);

    ismFragmentMembersWatchersBinding.rvMembersWatchers.setAdapter(membersAdapter);

    fetchMembers(false, null, false);
    ismFragmentMembersWatchersBinding.refresh.setOnRefreshListener(() -> fetchMembers(false, null, true));
    ...
    ismFragmentMembersWatchersBinding.etSearch.addTextChangedListener(new TextWatcher() {
      ...
      public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (s.length() > 0) {
          fetchMembers(true, s.toString(), false);
        } else {
  ...
  private final RecyclerView.OnScrollListener membersOnScrollListener = new RecyclerView.OnScrollListener() {
    @Override
    ...
    public void onScrolled(@NotNull RecyclerView recyclerView, int dx, int dy) {
      ...
      membersPresenter.fetchMembersOnScroll(membersLayoutManager.findFirstVisibleItemPosition(),
          membersLayoutManager.getChildCount(), membersLayoutManager.getItemCount());
    }
  };

  private void fetchMembers(boolean isSearchRequest, String searchTag, boolean showProgressDialog) {
    if (showProgressDialog) {
      showProgressDialog(getString(R.string.ism_fetching_conversation_members));
    }

    try {
      membersPresenter.fetchMembers(0, true, isSearchRequest, searchTag);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  @Override
  public void onMembersFetchedSuccessfully(ArrayList<MembersWatchersModel> membersModels, boolean refreshRequest) {
    if (refreshRequest) {
      members.clear();
    }
    members.addAll(membersModels);
    if (getActivity() != null) {
      getActivity().runOnUiThread(() -> {
        if (refreshRequest) {
          if (members.size() > 0) {
            ...
            ismFragmentMembersWatchersBinding.tvNoMembersWatchers.setText(getString(R.string.ism_no_members));
    ...
    if (getActivity() != null) {
      new AlertDialog.Builder(getActivity()).setTitle(
          ...
          .setPositiveButton(getString(R.string.ism_continue), (dialog, id) -> {
            ...
            membersPresenter.kickOutMembers(conversationId, Collections.singletonList(memberId));
          })
    ...
    if (getActivity() != null) {
      new AlertDialog.Builder(getActivity()).setTitle(
          ...
          .setPositiveButton(getString(R.string.ism_continue), (dialog, id) -> {
            ...
            membersPresenter.makeAdmin(conversationId, memberId);
          })
          .setNegativeButton(getString(R.string.ism_cancel), (dialog, id) -> dialog.cancel())
          .create()
          .show();
    ...
    if (getActivity() != null) {
      new AlertDialog.Builder(getActivity()).setTitle(
          ...
          .setPositiveButton(getString(R.string.ism_continue), (dialog, id) -> {
            ...
            membersPresenter.revokeAdminPermissions(conversationId, memberId);
          })
          .setNegativeButton(getString(R.string.ism_cancel), (dialog, id) -> dialog.cancel())
          .create()
          .show();
